...
import { batchCategorise } from '@/categorisation/ai';
...
const makeResponse = (results: Array<{ id: string; category: string }>) => ({
  choices: [{
    message: {
      tool_calls: [{
        type: 'function',
...
it('returns one result per transaction with category from AI', async () => {
...
it('passes category names from DB as the allowed enum values', async () => {
  ...
  mockCreate.mockResolvedValueOnce(makeResponse([{ id: 'txn-1', category: 'Groceries' }]));

  await batchCategorise([TRANSACTIONS[0]]);

  const callArgs = mockCreate.mock.calls[0][0];
  const toolSchema = callArgs.tools[0].function.parameters;
...
it('chunks 90 transactions into 3 API calls of 40/40/10', async () => {
  ...
  mockCreate
    .mockResolvedValueOnce(makeResponse(txns.slice(0, 40).map(t => ({ id: t.id, category: 'Shopping' }))))
    .mockResolvedValueOnce(makeResponse(txns.slice(40, 80).map(t => ({ id: t.id, category: 'Shopping' }))))
    .mockResolvedValueOnce(makeResponse(txns.slice(80).map(t => ({ id: t.id, category: 'Shopping' }))));
...
it('continues remaining chunks when one chunk throws', async () => {
  ...
  mockCreate
    ...
    .mockResolvedValueOnce(makeResponse(txns.slice(40).map(t => ({ id: t.id, category: 'Shopping' }))));
